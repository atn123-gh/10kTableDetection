...
from hpu_util.utils import DetectionCheckpointer_hpu, PeriodicCheckpointer_hpu
...
import hpu_util.utils as hutils
...
def do_test(cfg, model):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        # evaluator = COCOEvaluator(dataset_name, os.path.join(cfg.OUTPUT_DIR, "inference", dataset_name))   
        evaluator = get_evaluator(
            cfg, dataset_name, os.path.join(cfg.OUTPUT_DIR, "inference", dataset_name)
        )
        print(dataset_name)
        print(os.path.join(cfg.OUTPUT_DIR, "inference", dataset_name))
...
def do_train(cfg, model, resume=False):
    ...
    # if cfg.MODLE.DEVICE="hpu"
    periodic_checkpointer = PeriodicCheckpointer_hpu(
...
def main(args,cfg):
    ...
    if args.dl_worker_type == "HABANA" and cfg.MODEL.DEVICE=='hpu':
        try:
            import habana_dataloader
            data_loader_type=habana_dataloader
        except ImportError:
    ...
    logger.info("Model:\n{}".format(model))
    if args.eval_only:
        DetectionCheckpointer_hpu(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
    ...
    # logger.info("**args.distrubuted  {} ".format(args.distrubuted))
...
if __name__ == "__main__":
    ...
    if cfg.MODEL.DEVICE=='hpu' and distributed:
        hutils.init_distributed_mode(args)
    else:       # cuda/cpu
        launch(
            main,
            args.num_gpus,
